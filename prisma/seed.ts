...
async function main() {
  // 1. Create the pharmacy first — everything else needs its id
  ...
  const superpasswordHash = await bcrypt.hash('Super@123', 10);
  const passwordHash = await bcrypt.hash('Admin@123', 10);

  const superAdmin = await prisma.user.upsert({
    where: { pharmacyId: pharmacy.id, username: 'superadmin' },
    update: {},
    create: {
      username: 'superadmin',
  ...
  const admin = await prisma.user.upsert({
    where: { pharmacyId: pharmacy.id, username: 'admin' },
    update: {},
    create: {
      username: 'admin',
      passwordHash,
  ...
  const pharmacist = await prisma.user.upsert({
    where: { pharmacyId: pharmacy.id, username: 'pharmacist' },
    update: {},
    create: {
      username: 'pharmacist',
  ...
  const pharmacist2 = await prisma.user.upsert({
    where: { pharmacyId: pharmacy.id, username: 'pharmacist2' },
    update: {},
    create: {
  ...
  for (const item of products) {
    const exists = await prisma.product.findFirst({
      where: { barcode: item.barcode, pharmacyId: pharmacy.id },
    });
    ...
    await prisma.product.create({
      data: {
        ...
        pharmacy: {
          connect: { id: pharmacy.id },   // relational form, not pharmacyId: pharmacy.id
        },
      },
    });
  ...
  console.log(`Seeded ${products.length} products for pharmacy: ${pharmacy.name}`);
}
