...
// // Recompute every line server-side — never trust totals/GST split sent from the client.
...
//     // purchaseOrderId is nullable in the schema, but this endpoint enforces
//     // it at the app level: every invoice created here always gets a PO first.
...
//             // Make sure every referenced product actually exists before we touch anything else.
...
//             // Step 1: purchase order — created and marked RECEIVED in the same transaction
//             // since this form skips the separate "send PO, wait, receive later" flow.
...
//             // Step 2: purchase invoice (GRN) — always linked to the PO just created.
...
//             // Step 3: one Batch + one PurchaseInvoiceItem per line.
...
//             // Step 4: post to the GST input ledger.
...
export async function POST(req: NextRequest) {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            // Resolve location names sent from the form into Location ids.
            // Batch.location is a relation (locationId) now, not free text —
            // PurchaseInvoiceItem.location stays free text by design.
            ...
            for (let i = 0; i < data.items.length; i++) {
                ...
                const batch = await tx.batch.create({
                    data: {
                        ...
                        sellingPrice: item.sellingPrice,
                        quantityAvailable: item.quantity,
                        locationId: item.location ? locationIdByName.get(item.location) ?? null : null,
                    },
                });
