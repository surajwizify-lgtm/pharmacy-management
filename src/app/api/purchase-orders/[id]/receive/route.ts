...
//     // body: { invoiceNumber, invoiceDate, isInterState, items: [{ productId, quantity, batchNumber, manufactureDate?, expiryDate, purchaseRate, mrp, sellingPrice, discountPercent, gstPercentage, hsnCode, location? }] }
...
//             // mark PO received (simplification: treats every receive as full receipt;
//             // partial receiving would need per-item quantity tracking against po.items)
//             await tx.purchaseOrder.update({
//                 where: { id: po.id },
//                 data: { status: 'RECEIVED' },
//             });

//             // push to GST ledger (input tax)
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            //     body.items.map((it: any) => it.location).filter((v: any): v is string => !!v)
            // )];
