...
import { createSalesReturnSchema } from "@/lib/schemas";
import { createSalesReturn } from "@/lib/returns";
...
// POST /api/returns - ADMIN/PHARMACIST/CASHIER, mirrors POST /api/bills
export async function POST(req: NextRequest) {
    return withErrorHandling(async () => {
        await requireSession([Role.ADMIN, Role.PHARMACIST, Role.CASHIER]);
        const dto = createSalesReturnSchema.parse(await req.json());
        return createSalesReturn(dto);
    });
}