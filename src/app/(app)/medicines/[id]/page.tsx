...
export default function MedicineDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!medicine) return <p className="text-slate-400">Loading…</p>;

  const totalStock = medicine.batches.reduce((s, b) => s + b.quantityAvailable, 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-1 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">{medicine.name}</h1>
            <p className="text-sm text-slate-500">
              {medicine.manufacturer} · HSN {medicine.hsnCode} · GST {medicine.gstPercentage}%
            </p>
          </div>
          <span className={`badge ${medicine.status === 'ACTIVE' ? 'bg-brand-100 text-brand-700' : 'bg-slate-200 text-slate-500'}`}>
            {medicine.status}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="card p-4">
          ...
          <p className="text-xl font-semibold text-slate-900">{medicine.batches.length}</p>
        </div>
        <div className="card p-4">
          <p className="label mb-1">Prescription</p>
          <p className="text-xl font-semibold text-slate-900">{medicine.prescriptionRequired ? 'Required' : 'Not required'}</p>
        </div>
      </div>
