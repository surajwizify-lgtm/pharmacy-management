...
export default function BillingPage() {
  ...
  async function submitBill() {
    ...
    try {
      ...
      if ((estimatedSubtotal + estimatedGst) && Number(estimatedSubtotal + estimatedGst) > 0) {
        try {
          await apiFetch(`/api/bills/${bill.id}/payments`, {
            method: 'POST',
            body: JSON.stringify({ amount: Number(estimatedSubtotal + estimatedGst), method: createPaymentMethod }),
          });
        } catch (err) {
