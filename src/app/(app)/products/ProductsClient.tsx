...
export default function ProductsClient({
    ...
    const canEdit = role === 'ADMIN' || role === 'PHARMACIST' || role == Role.SUPER_ADMIN;
    ...
    useEffect(() => {
        // Skip the first run: server already provided the unfiltered initial list.
    ...
    const columns: ColumnDef<product>[] = [
        {
            ...
            header: "Product",
            cell: ({ row }) => {
                return <Link className='text-blue-500 hover:text-blue-700' href={cn('/products/', row.original.id)}>{row.original.name}</Link>
            },
        },
        {
        ...
        //     accessorKey: "cp",
        //     header: "CP",
        //     cell: ({ row }) => `₹${row.original.cp}`,
        // },
        // {
        //     accessorKey: "sp",
        //     header: "SP",
        //     cell: ({ row }) => `₹${row.original.sp}`,
        // },
        {
            id: "stock",
            header: "Stock",
            cell: ({ row }) => {
                const total = row.original.batches.reduce(
            ...
            cell: ({ row }) => {
                return (
                    <div className="flex gap-2">
                        {canEdit && (
                            <div className="flex">
                                <Button onClick={() => openEdit(row.original)} variant="ghost">
                                    <EditIcon />
                                </Button>
                                {(role === 'ADMIN' || role === Role.SUPER_ADMIN) && (
                                    <Button
                                        variant={row.original.status === 'ACTIVE' ? 'destructive' : 'default'}
                                        onClick={() => setPendingDiscontinue(row.original)}
                                    >
                                        {row.original.status === 'ACTIVE' && <BanIcon />}
                                        {row.original.status === 'ACTIVE' ? '' : 'Activate'}
                                    </Button>
                                )}
                            </div>
                        )}
    ...
    return (
        <div className="space-y-6">
            <PageHeader header="Products" subheader="Catalog, GST slabs, and stock overview.">
                    ...
                    <div className="flex gap-1.5">
                        {filterPills.map((pill) => (
                            <Button
                                ...
                                className={statusFilter === pill.key ? 'bg-gray-400' : ''}
                            >
                                {pill.label}
                            </Button>
                        ))}
                                    ...
                                    return (
                                        <TableRow key={m.id}>
                                            <TableCell className="whitespace-normal">
                                                <Link href={`/products/${m.id}`} className="font-medium text-neutral-900 hover:text-primary-600">
                                                    {m.name}
                                                </Link>
                                            ...
                                            <TableCell>
                                                <span
                                                    className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-medium ${m.status === 'ACTIVE' ? 'bg-secondary-100 text-secondary-700' : 'bg-neutral-200 text-neutral-500'
                                                        }`}
                                                >
                                                    <span
                                            ...
                                            <TableCell>
                                                {canEdit && (
                                                    <div className="grid grid-cols-2 gap-1">
                                                        <Button onClick={() => openEdit(m)} variant="ghost">
                                                            <EditIcon />
                                                        </Button>
                                                        {role === 'ADMIN' && (
                                                            <Button
            ...
            <ProductFormModal open={showForm} onClose={() => setShowForm(false)} product={editingProduct} onSuccess={load} />
            <ConfirmDialog
